...
```
Agent(
  subagent_type: "docs-organizer",   // .Codex/agents/docs-organizer.md (haiku 고정)
  description: "문서 최신화",
...
> 같은 작업을 시키고, 에이전트 정의 파일([.Codex/agents/docs-organizer.md](.Codex/agents/docs-organizer.md))의
> 지침을 그대로 전달한다.
...
- **자동 게시(검증 통과분만).** 일일 파이프라인은 봇이 수집·분석 후 `backend/scripts/auto-build.mjs`로
  **출처를 자동검증**해 통과한 항목만 `backend/data/trends.json`에 반영하고 **git push(자동 배포)**한다.
  ...
  · 일일 에이전트 크론: **21:30 `한끗 자동 수집·게시`**(hangeut-run→auto-build). 요일 로테이션 → **목 = 신조어**(주간 사전 갱신). 저녁 시간대(아침엔 PC가 꺼져 있어서).
  · **503 자동복구.** Gemini가 21:30에 용량초과(503/429)로 죽는 날을 위해, **21:50 `한끗 수집 재시도 게이트`**(no-agent, `hangeut_retry_gate.py`)가 그날 수집이 `error`면 1회 재실행(성공·미실행이면 no-op). ⚠️ 게이트는 22:00→21:50으로 당김(2026-06-23): **PC가 21:35쯤 꺼져** 22:00 게이트가 거의 안 떴음(06-20 이후 미실행) → 21:30 503난 날 복구 실패가 "화요일 누락"의 진짜 원인이었음. 근본 해결은 **PC를 22:30까지 켜두기**(또는 수동 백필). 503은 모델 무관 — GA(2.5-pro)·유료키로도 21:30엔 남.
  · **모델: `gemini-2.5-pro`(GA) · provider `gemini`(API키, 유료 Tier-1) 단독**(2026-06-23 확정). 프리뷰(gemini-3-flash-preview)는 21:30 503 잦고, OAuth(cloudcode) 경로는 병렬 툴콜 400 버그라 폐기. 2.5-pro는 실수집 검증 OK(에이전트 다중 툴콜 정상).
- **광고/진짜 일일 재확인.** "광고일까 진짜일까"는 카테고리와 무관하게 **매일 1건**(가장 오래된 신뢰분석)을
  ...
  · 일일 no-agent 크론: **21:15 `한끗 광고/진짜 일일 재확인`**(recheck_ad.py→recheck-ad.mjs). 메인 수집(21:30)보다 먼저 끝나 trends.json 쓰기 충돌 없음(결정적·빠름).
...
- **API 키·토큰을 코드·문서·커밋에 남기지 않는다.** 비밀은 `.Codex/`·`.env`(둘 다 gitignore)에만.
...
- Hermes(고구미봇): 별도 레포 `E:\workspace\side_project\hermes`. cron은 **전부 KST 21~22시대(저녁, PC 켜진 시간대)** — 펄스/수집 분야 안내 21:00, 주간 갱신 월 21:00, 광고/진짜 재확인 21:15, 메인 `한끗 자동 수집·게시` **21:30**(목=신조어 주간 사전), **수집 재시도 게이트 22:00**(503 실패시에만 같은 모델 재실행). ⚠️ Hermes는 로컬이라 **그 시각에 PC+게이트웨이가 켜져 있어야** 실행됨(`tools/start-hermes.bat`). PC는 보통 ~22:30까지만 켜짐.
