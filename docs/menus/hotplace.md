...
- **`t.shops` (선택, "어디서 사 먹지?" 섹션)** — 방문 가능한 핫플레이스 또는 카페 주소 목록 `[{name, area?, url?, note?}]`. 지역·예약링크·영업시간 등. auto-build.mjs가 각 url 생존성 검증(404/410/ERR 제거)
...
- **가게 목록(shops):** 상세 페이지 중단부에 "어디서 사 먹지?"로 표시. 예약링크·지도링크·공식 페이지 등 본문이 읽히는 링크. auto-build.mjs가 각 url 생존성 검증해 죽은 링크 제거 (이름·지역·메모는 유지)
