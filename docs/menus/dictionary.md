...
- **`t.pureKorean`** (우리말 대체어, 존재 시 뱃지 표시) — `assets/app.js` L114 참고
- `t.excerpt` / `t.verdict` (장문 설명)
- `t.src` (출처 URL)
- `t.recs` (추천 목록)

## 현재 상태
구현됨. trend.html에서 타입별 렌더링 분기 동작 (신뢰분석 vs 트렌드). 신조어 항목 3개 이상(이왜진→난리자베스 교체됨) 존재.

## 관련 코드
- `trend.html` : 상세 페이지 마크업 (L1~80+)
- `assets/app.js` : H.coverHTML() 신조어 뱃지 렌더링 (L102~109)
- `data/trends.json` : type=='트렌드' 항목들
