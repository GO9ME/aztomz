...
- `t.src` (출처 URL 배열)
- **`t.shops` (선택, "어디서 사 먹지?" 섹션)** — 파는 가게 목록 `[{name, area?, url?, note?}]`. 카페 4곳 + 편의점처럼 실제 구매 가능한 곳들. auto-build.mjs가 각 url 생존성 검증(404/410/ERR 제거)
...
- **가게 목록(shops):** 상세 페이지 중단부에 "어디서 사 먹지?"로 표시. 인스타/다이닝코드 profile 금지, 실제 검색에 나온 본문이 읽히는 링크만(tistory·블로그·기사·trip.com). auto-build.mjs가 각 url 생존성 검증해 죽은 링크 제거 (이름·지역·메모는 유지)
