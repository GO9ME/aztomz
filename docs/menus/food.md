...
## 관련 코드
- `trend.html` : 상세 페이지 (L1~80+, scoresBlock() 함수로 신뢰분석/트렌드 분기)
- `assets/app.js` : H.bandTxt() (점수 설명 텍스트), H.coverHTML() (카버 렌더링) (L29, L102~109)
- `data/trends.json` : type=='신뢰분석' || type=='트렌드' 중 cat 포함 음식점·카페 항목

## 비고
- **카테고리 예:** 디저트, 성수 카페, 베이커리 등 — coverCat으로 스타일링 (cat-dessert, cat-cafe, cat-bakery)
- 신뢰분석: 광고/신뢰도 점수 + 바차트 렌더링 (assets/app.js L56~62)
- 트렌드: 단계(stage) + 단계 설명(stageMsg) 표시
- 출처 검증 필수: `node scripts/check-links.mjs` + `check-source.mjs` (learnings.md 참고)
